#!/usr/bin/env node
/**
 * CLI 入口：将工具调用转发至 FastAPI Server（境外全网搜版本）。
 *
 * 用法：node scripts/cli.js search --cx posts --q "AI" --num 10
 *       node scripts/cli.js stats --q "AI" --metrics '[{"type":"hotWords","topN":10}]'
...
async function search({ cx, q, num = 10, start = 1, sort, fields } = {}) {
  const params = new URLSearchParams({ key: API_KEY, cx, q });
  ...
  return forwardGet(`/international-search/v1?${params.toString()}`);
}
...
async function stats({ q = null, cx = null, metrics = [] } = {}) {
  ...
  // 启动 stats 请求（可能耗时数分钟）
  const statsPromise = forwardPost("/api/intl-stats", body);
  ...
  const pollProgress = async () => {
    ...
    while (polling) {
      try {
        const raw = await forwardGet("/api/intl-stats/progress");
...
const HELP_TEXT = `istarshine-internation-web-wide-search — 境外全网搜 CLI
...
工具:
  ...
  stats         统计分析（热词、话题、情感、趋势、平台对比、语言分布等，支持一次多指标）

search 参数:
  --cx <string>      (必填) posts 或 comments
  --q <string>       (必填) 查询字符串，支持运算符语法（site:、dateRestrict:、author:、sentiment:、lang: 等）
...
stats 参数:
  ...
  --metrics <json>   (必填) 统计指标 JSON 数组，如 '[{"type":"hotWords","topN":10},{"type":"sentiment"}]'
                     支持指标: hotWords, hotHashtags, sentiment, trend, platformCompare, langDistribution

示例:
  node scripts/cli.js search --cx posts --q "AI" --num 10
  node scripts/cli.js search --cx comments --q "AI lang:en dateRestrict:d7" --num 5
  node scripts/cli.js stats --q "AI dateRestrict:d7" --metrics '[{"type":"hotWords","topN":10},{"type":"langDistribution"}]'
`;
