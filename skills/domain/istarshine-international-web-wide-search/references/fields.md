...
| 字段 | 类型 | 说明 |
|------|------|------|
| `lang_code` | keyword | 语言代码（如 en、ja、ko、ar、fr、de、es、ru、pt、th 等），标识帖文的原始语言 |
| `translation` | text | 中文翻译内容，存储帖文的中文翻译文本 |
| `channel` | keyword | 频道分类（如科技、财经、体育、娱乐等） |
| `title` | text | 标题 |
| `content` | text | 正文内容 |
| `ocr` | text | 图片 OCR 文本 |
...
| `gather.site_domain` | keyword | 平台域名 |
| `gather.site_name` | keyword | 网站/平台名称（如快手、新华网、Twitter、BBC 等） |
...
| `user.gender` | keyword | 性别 |
| `user.verified` | integer | 认证状态 |
| `user.verified_type` | integer | 认证类型 |
...
> **境外全网搜特有字段**：`lang_code` 和 `translation` 是境外全网搜相比国内全网搜新增的字段。`lang_code` 标识帖文的原始语言，`translation` 存储帖文的中文翻译内容。境外数据不包含 `label.label_ids` 字段（无 MCN/采编权标签服务）。
