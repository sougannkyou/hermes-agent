...
| `site:` | 平台域名过滤（支持所有平台域名，不限于特定平台） | `site:kuaishou.com`、`site:xiaohongshu.com`、`site:iesdouyin.com`、`site:weibo.com` |
| `url:` | 帖文链接精确匹配 | `url:https://www.example.com/post/xxx` |
...
| `lang:` | 语言代码过滤，按帖文原始语言筛选（如 en、ja、ko、ar、fr、de 等） | `lang:en`、`lang:ja`、`lang:ko` |
...
| `-` 前缀 | 排除条件（可用于任何运算符） | `-site:weibo.com`、`-lang:ar` |

范围筛选语法：`min..max`（双边）、`min..`（仅下限）、`..max`（仅上限）、`数值`（等价于 ≥ 该值）。

未识别的运算符会被当作普通关键词处理（同样强制精确匹配）。

> **境外全网搜说明**：`lang:` 运算符是境外全网搜特有的运算符，用于按语言代码过滤数据。`site:` 运算符支持所有境外社交媒体平台域名。不使用 `site:` 运算符时，搜索将覆盖所有平台的数据。
...
```
AI lang:en dateRestrict:d7
```
搜索所有平台最近 7 天内英语的包含"AI"的内容。

```
AI lang:ja site:twitter.com dateRestrict:d7
```
搜索 Twitter 平台最近 7 天内日语的包含"AI"的内容。

```
AI "artificial intelligence" -lang:ar author:张三 sentiment:positive
```
搜索作者为"张三"、情感为正面、排除阿拉伯语的包含"AI"和"artificial intelligence"的内容。

```
新能源 followers:500000.. lang:ko dateRestrict:d7
```
搜索最近 7 天内、粉丝数 ≥ 50 万的大V发布的韩语包含"新能源"的内容。

```
小米汽车 replies:10000.. likes:5000.. lang:en
```
搜索所有平台中评论数 ≥ 1 万且点赞数 ≥ 5000 的英语包含"小米汽车"的热门内容。
