...
> **全网搜说明**：`site:` 运算符支持所有中国境内社交媒体平台域名，不限于 `iesdouyin.com` 和 `weibo.com`。不使用 `site:` 运算符时，搜索将覆盖所有平台的数据。
...
新能源 dateRestrict:d7
```
搜索所有平台最近 7 天内包含"新能源"的内容。

```
新能源 site:kuaishou.com dateRestrict:d7
```
搜索快手平台最近 7 天内包含"新能源"的内容。
...
```
搜索所有平台中评论数 ≥ 1 万且点赞数 ≥ 5000 的包含"小米汽车"的热门内容。
