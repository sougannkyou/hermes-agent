#!/usr/bin/env node
/**
 * CLI 入口：将工具调用转发至 FastAPI Server（全网搜版本）。
 *
 * 用法：node scripts/cli.js search --cx posts --q "新能源" --num 10
...
/**
 * 推断时间范围层级。
 * 优先级：startDate/endDate > dateRestrict: 运算符 > 默认 30d
 * @param {object} params - 请求参数
 * @returns {"30d" | "365d" | "all"} 时间范围层级
 */
function _inferTier(params) {
  const { startDate, endDate, q } = params || {};

  // 1. 从 startDate/endDate 计算天数（仅 startDate 存在时才走此分支）
  if (startDate) {
  ...
  // 2. 从 q 中解析 dateRestrict: 运算符
  if (q) {
    const match = String(q).match(/dateRestrict:([hdwm])(\d+)/i);
    if (match) {
      const unit = match[1].toLowerCase();
      const n = parseInt(match[2], 10);
      let days;
      switch (unit) {
        case "h": days = Math.ceil(n / 24); break;
        case "d": days = n; break;
  ...
  // 3. 默认 30d（包括仅指定 endDate 无 startDate 的情况）
  return "30d";
}
...
const TIER_ENDPOINTS = {
  search: {
    "30d":  "/widesearch/v1",
    "365d": "/widesearch-365/v1",
    "all":  "/widesearch-all/v1",
  },
  stats: {
    "30d":  "/api/wide-stats",
    "365d": "/api/wide-stats-365",
    "all":  "/api/wide-stats-all",
  },
  progress: {
    "30d":  "/api/wide-stats/progress",
    "365d": "/api/wide-stats-365/progress",
    "all":  "/api/wide-stats-all/progress",
  },
};

// ---------- 工具实现 ----------

async function search(allParams = {}) {
  const { cx, q, num = 10, start = 1, sort, fields } = allParams;
  const tier = _inferTier(allParams);
  const endpoint = TIER_ENDPOINTS.search[tier];
  ...
  return forwardGet(`${endpoint}?${params.toString()}`);
}
...
async function stats(allParams = {}) {
  const { q = null, cx = null, metrics = [] } = allParams;
  const tier = _inferTier(allParams);
  const statsEndpoint = TIER_ENDPOINTS.stats[tier];
  const progressEndpoint = TIER_ENDPOINTS.progress[tier];
  ...
  // 启动 stats 请求（可能耗时数分钟）
  const statsPromise = forwardPost(statsEndpoint, body);
  ...
  const pollProgress = async () => {
    ...
    while (polling) {
      try {
        const raw = await forwardGet(progressEndpoint);
...
const HELP_TEXT = `istarshine-domestic-web-wide-search — 中国境内全网搜 CLI
...
示例:
  node scripts/cli.js search --cx posts --q "新能源" --num 10
